...
def download_file_unzip(url, output_path, extract_path='.'):
    # 파일이 이미 존재하는지 확인
    if not os.path.exists(output_path) and not os.path.exists('data'):
        ...
        # 압축 해제 후 tar.gz 파일 삭제
        print(f"Removing {output_path}...")
        os.remove(output_path)
        print(f"Removed {output_path}")
    elif os.path.exists(output_path) and not os.path.exists('data'):
        print(f"Extracting {output_path}...")
        with tarfile.open(output_path, "r:gz") as tar:
            tar.extractall(path=extract_path)
        print(f"Extracted to {output_path}")
        ...
        print(f"File {output_path} already exists. Skipping download & unzip")
